...
ngOnInit() {
  this.productsRequestService.getProductsList().subscribe((data) => {
    ...
    console.log(data);
    let product = this.products.find((product: Product) => product.id == this.activatedRoute.snapshot.params['id']);
  ...
  productDetails: any;
  constructor(private activatedRoute: ActivatedRoute, private productsRequestService: ProductsRequestService, private cartService: CartService){
